//
//  QrcodeDescriptorPlugin.m
//  qrcode_descriptor
//
//  Created by Yue on 2021/7/1.
//

#import "QrcodeDescriptorPlugin.h"
#import <CoreImage/CoreImage.h>

@implementation QrcodeDescriptorPlugin

+ (void)registerWithRegistrar:(nonnull id<FlutterPluginRegistrar>)registrar {
    FlutterMethodChannel *channel = [FlutterMethodChannel methodChannelWithName:@"qrcode_descriptor" binaryMessenger:registrar.messenger];
    QrcodeDescriptorPlugin *instance = [[QrcodeDescriptorPlugin alloc] init];
    [registrar addMethodCallDelegate:instance channel:channel];
}

- (void)handleMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result {
    if ([@"getPlatformVersion" isEqualToString:call.method]) {
        result([@"macOS -OC " stringByAppendingString:[NSProcessInfo processInfo].operatingSystemVersionString]);
    }
